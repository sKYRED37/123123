...
OWNER_ID  = int(os.getenv("OWNER_ID", "1766395031"))
...
async def db_update_map_stats(team_name: str, map_name: str, won: bool) -> float:
    ...
    ТУРНИРНОГО матча значение немного сдвигается (вверх при победе, вниз при
    поражении) на небольшую случайную величину, оставаясь в рамках 70–100 —
    плавное колебание от матча к матчу, а не фиксированное число."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
...
async def cmd_admin(msg: Message) -> None:
    ...
    if is_admin:
        ...
        await msg.answer(
            ...
            "и сбросить лимит подписаний свободных агентов\n"
            "<code>/deltour [тур]</code> — удалить турнир и всю его статистику\n"
            "<code>/tourprize [команда] [тур] [место]</code> — приз за 1–3 место\n"
            "<code>/tourprizeall [тур] / 1 Команда / 2 Команда / 3 Команда</code> — призы за 1–3 место разом\n"
            "<code>/tourteams [тур] / Команда1 / Команда2 / ...</code> — зарегистрировать участников турнира\n"
            "<code>/tourprizerest [тур] / Команда1 / Команда2 / ...</code> — приз «остальным» командам турнира\n"
        )
        await msg.answer(
...
async def db_set_tournament_participants(tour_name: str, team_names: list[str]) -> tuple[int, list[str]]:
    """Регистрирует список команд-участников турнира (INSERT OR IGNORE —
    не дублирует, если часть уже была добавлена раньше). Команды, которых
    нет в лиге, пропускаются. Возвращает (сколько реально добавлено,
    список пропущенных ненайденных названий)."""
    added = 0
    skipped: list[str] = []
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        for team_name in team_names:
            ...
            _, real_name, *_ = team
            cursor = await db.execute(
                "INSERT OR IGNORE INTO tournament_participants (tour_name, team_name, added_at) VALUES (?,?,?)",
                (tour_name, real_name, now)
            )
...
async def db_get_tournament_participants(tour_name: str) -> list[str]:
    """Список команд, зарегистрированных как участники турнира."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT team_name FROM tournament_participants WHERE tour_name=? COLLATE NOCASE ORDER BY id",
            (tour_name,)
        ) as cursor:
            rows = await cursor.fetchall()
    return [r[0] for r in rows]
...
async def db_get_tournament_podium_teams(tour_name: str) -> set[str]:
    """Команды, которым уже записано место 1–3 на этом турнире
    (team_tournament_places)."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT team_name FROM team_tournament_places WHERE tour_name=? COLLATE NOCASE AND place IN (1,2,3)",
            (tour_name,)
        ) as cursor:
            rows = await cursor.fetchall()
    return {r[0] for r in rows}
...
@dp.message(Command("tourteams"))
async def cmd_tourteams(msg: Message) -> None:
    """
    /tourteams Турнир / Команда1 / Команда2 / ... / Команда16
    Регистрирует список команд-участников турнира (один раз на турнир, можно
    дополнять позже — уже добавленные команды не дублируются). Этот список
    нужен, чтобы /tourprizerest могла сама вычислить «остальные» команды
    (участники минус занявшие 1–3 место), не требуя перечислять их вручную
    каждый раз. Только для администраторов.
    """
    if not await db_is_admin(msg.from_user.id):
        await msg.answer("⛔ Только администратор может регистрировать участников турнира.")
        return

    raw = msg.text.split(maxsplit=1)
    USAGE = (
        "❌ Формат: <code>/tourteams Турнир / Команда1 / Команда2 / ...</code>\n\n"
        "Перечисли через «/» ВСЕ команды-участники турнира (обычно 16, "
        "для SPL Pro League LAN Finals — 8). Можно вызывать несколько раз, "
        "чтобы дополнить список — уже добавленные команды не дублируются.\n\n"
        "Пример:\n"
        "<code>/tourteams Winline Epic Standoff 2 Jumble Rumble S1 / NaVi / "
        "Spirit / Aurora / Virtus.pro / ...</code>"
    )
    if len(raw) != 2 or raw[1].count("/") < 1:
        await msg.answer(USAGE)
        return
    ...
    fields = [p.strip() for p in raw[1].split("/")]
    tour_name = fields[0]
    team_names = [t for t in fields[1:] if t]

    if not tour_name or not team_names:
        await msg.answer(USAGE)
        return

    added, skipped = await db_set_tournament_participants(tour_name, team_names)
    total_now = len(await db_get_tournament_participants(tour_name))

    lines = [
        f"✅ <b>Участники турнира «{md_escape(tour_name)}» обновлены.</b>\n",
        f"Добавлено новых: <b>{added}</b>  |  Всего в списке: <b>{total_now}</b>",
    ]
    if skipped:
        lines.append(
            "\n⚠️ Не найдены в лиге и пропущены: "
            + ", ".join(f"«{md_escape(s)}»" for s in skipped)
        )
    await safe_send_long(msg, "\n".join(lines))


@dp.message(Command("tourprizerest"))
async def cmd_tourprizerest(msg: Message) -> None:
    """
    /tourprizerest Турнир — автоматически: делит фонд «остальных» команд
    турнира поровну между всеми зарегистрированными через /tourteams
    участниками, которые ещё НЕ занимали место 1–3 (team_tournament_places).
    /tourprizerest Турнир / Команда1 / Команда2 / ... — вручную: делит фонд
    между явно перечисленными командами (как раньше, если участники турнира
    не регистрировались через /tourteams).
    Только для администраторов.
    """
    if not await db_is_admin(msg.from_user.id):
        await msg.answer("⛔ Только администратор может начислять призовые за турниры.")
    ...
    USAGE = (
        "❌ Формат:\n"
        "<code>/tourprizerest Турнир</code> — автоматически, по списку "
        "участников из <code>/tourteams</code> (минус занявшие топ-3)\n"
        "<code>/tourprizerest Турнир / Команда1 / Команда2 / ...</code> — "
        "вручную, явным списком команд\n\n"
        "Пример:\n"
        "<code>/tourprizerest Winline Epic Standoff 2 Jumble Rumble S1</code>"
    )
    ...
    if auto_mode:
        participants = set(await db_get_tournament_participants(tour_name))
        if not participants:
            await msg.answer(
                f"❌ Для турнира «{md_escape(tour_name)}» ещё не зарегистрированы участники.\n\n"
                f"Сначала: <code>/tourteams {md_escape(tour_name)} / Команда1 / Команда2 / ...</code>\n"
                f"Либо укажи команды вручную: <code>/tourprizerest {md_escape(tour_name)} / "
                f"Команда1 / Команда2 / ...</code>"
            )
            return
        podium = await db_get_tournament_podium_teams(tour_name)
        team_names = sorted(participants - podium)
        if not team_names:
            await msg.answer(
                f"❌ Все зарегистрированные участники турнира «{md_escape(tour_name)}» "
                f"уже отмечены как призёры (1–3 место) — раздавать «остальным» некому."
            )
            return

    pool = tournament_remaining_pool(tour_name)
    if pool <= 0:
        await msg.answer(
            f"❌ Для турнира «{md_escape(tour_name)}» призовой фонд на «остальные» "
            f"команды не задан (0 ₽) — начислять нечего."
        )
        return

    per_team = tournament_remaining_per_team(tour_name, teams_count=len(team_names))
    mode_note = "автоматически, по списку участников" if auto_mode else "вручную указанным командам"
    pool_note = f" (из фонда {fmt_money(pool)} на {len(team_names)} команд, {mode_note})"
    ...
    for team_name in team_names:
        ...
        if not team:
            ...
            continue
        ok, err, new_balance = await db_add_balance(
            team_name, float(per_team),
            f"Призовые за участие на турнире «{tour_name}»{pool_note}",
            msg.from_user.id
        )
        if not ok:
            results.append(f"❌ {md_escape(team_name)}: {err}")
            continue
        results.append(
            f"🎖 <b>{md_escape(team_name)}</b> — +{fmt_money(per_team)} "
            f"(баланс: {fmt_money(new_balance)})"
        )

    lines = [
        f"💰 <b>Призовые «остальным» на турнире «{md_escape(tour_name)}» выданы!</b>{md_escape(pool_note)}\n",
        *results,
    ]
    await safe_send_long(msg, "\n".join(lines))
...
async def cmd_tours(msg: Message) -> None:
    ...
    for name, is_done, created_at in rows:
        ...
        if pool is not None:
            ...
            lines.append(f"    💰 Призовой фонд (топ-3): <b>{fmt_money(pool)}</b>")
            lines.append(
                f"    🏆 Выдача призовых: 🥇 {fmt_money(breakdown[1])}  "
                f"🥈 {fmt_money(breakdown[2])}  🥉 {fmt_money(breakdown[3])}"
            )
            rest_pool = tournament_remaining_pool(name)
            if rest_pool > 0:
                rest_slots = tournament_remaining_slots(name)
                rest_each = tournament_remaining_per_team(name)
                lines.append(
                    f"    🎖 Фонд на остальные команды: <b>{fmt_money(rest_pool)}</b> "
                    f"(÷{rest_slots} ≈ {fmt_money(rest_each)} каждой)"
                )
        else:
    ...
    lines.append("\n<i>Чтобы выдать призовые «остальным» командам турнира:</i>")
    lines.append("<code>/tourprizerest Турнир</code> (авто, по /tourteams) "
                 "или <code>/tourprizerest Турнир / Команда1 / ...</code> (вручную)")
...
# Число команд-участников, призовой фонд за 1–3 место и призовой фонд на
# оставшиеся (не призовые) команды турнира — в рублях, по каждому турниру
# сезона. Во всех турнирах участвует 16 команд, кроме LAN-финала SPL Pro
# League (8 команд). Фонд за 1–3 место распределяется в пропорции
# 50% / 30% / 20% — см. tournament_prize_breakdown(). Фонд на оставшиеся
# команды делится поровну между всеми участниками, не попавшими в топ-3 —
# см. tournament_remaining_per_team().
TOURNAMENT_PRIZE_POOL: dict[str, tuple[int, int, int]] = {
    "Winline Epic Standoff 2 Jumble Rumble S1": (16, 2_500_000, 2_000_000),
    "CONDR Season 1": (16, 750_000, 0),
    "Winline Epic Standoff 2 Jumble Rumble S2": (16, 2_500_000, 2_000_000),
    "CONDR Season 2": (16, 750_000, 0),
    "ES1ZE Cup S1": (16, 500_000, 0),
    "SPL Pro League Season 1": (16, 2_000_000, 2_000_000),
    "Winline Epic Standoff 2 Jumble Rumble S3": (16, 2_500_000, 2_000_000),
    "CONDR Season 3": (16, 750_000, 0),
    "Winline Epic Standoff 2 Jumble Rumble S4": (16, 2_500_000, 2_000_000),
    "CONDR Season 4": (16, 750_000, 0),
    "SPL Pro League Season 2": (16, 2_000_000, 2_000_000),
    "ES1ZE Cup S2": (16, 500_000, 0),
    "Winline Major Standoff 2 Jumble Rumble S1": (16, 5_500_000, 5_000_000),
    "Winline Epic Standoff 2 Jumble Rumble S5": (16, 2_500_000, 2_000_000),
    "CONDR Season 5": (16, 750_000, 0),
    "SPL Pro League Season 3": (16, 2_000_000, 2_000_000),
    "Winline Epic Standoff 2 Jumble Rumble S6": (16, 2_500_000, 2_000_000),
    "CONDR Season 6": (16, 750_000, 0),
    "SPL Pro League Season 4": (16, 2_000_000, 2_000_000),
    "ES1ZE Cup S3": (16, 500_000, 0),
    "Winline Epic Standoff 2 Jumble Rumble S7": (16, 2_500_000, 2_000_000),
    "CONDR Season 7": (16, 750_000, 0),
    "SPL Pro League Season 5": (16, 2_000_000, 2_000_000),
    "Winline Epic Standoff 2 Jumble Rumble S8": (16, 2_500_000, 2_000_000),
    "CONDR Season 8": (16, 750_000, 0),
    "SPL Pro League LAN Finals 1": (8, 5_000_000, 5_000_000),
    "ES1ZE Cup S4": (16, 500_000, 0),
    "Winline Major Standoff 2 Jumble Rumble S2": (16, 5_000_000, 5_000_000),
}


def _find_prize_pool_entry(tour_name: str) -> tuple[int, int, int] | None:
...
def tournament_remaining_pool(tour_name: str) -> int:
    """Общий призовой фонд турнира на команды, не попавшие в топ-3
    (делится поровну между ними). 0, если для турнира такой фонд не задан
    или он не найден в TOURNAMENT_PRIZE_POOL."""
    entry = _find_prize_pool_entry(tour_name)
    return entry[2] if entry else 0


def tournament_remaining_slots(tour_name: str) -> int:
    """Сколько команд турнира не попадает в топ-3 (число участников минус 3,
    не меньше нуля)."""
    return max(0, tournament_teams_count(tour_name) - 3)


def tournament_remaining_per_team(tour_name: str, teams_count: int | None = None) -> float:
    """Призовые на одну команду из фонда «остальных» команд турнира,
    поделённого поровну. По умолчанию делит на (число участников − 3);
    если явно передано teams_count (например, реальное число команд,
    которым начисляем) — делит на него вместо дефолтного расчёта."""
    pool = tournament_remaining_pool(tour_name)
    if pool <= 0:
        return 0.0
    n = teams_count if teams_count is not None else tournament_remaining_slots(tour_name)
    if n <= 0:
        return 0.0
    return pool / n
